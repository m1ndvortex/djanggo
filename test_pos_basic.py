#!/usr/bin/env python
"""
Basic test script for POS backend functionality.
"""
import os
import django
...
def test_pos_backend():
    ...
    print("🚀 Testing POS Backend Functionality (Basic)")
    print("=" * 50)
    ...
    try:
        ...
        with tenant_context(tenant):
            ...
            # Test 2: Create test user and customer with unique identifiers
            print("\n2. Creating test user and customer...")
            
            unique_id = str(uuid.uuid4())[:8]
            
            user = User.objects.create_user(
                username=f'testpos_{unique_id}', 
                password='test123', 
                role='salesperson'
            )
            ...
            customer = Customer.objects.create(
                first_name='Test',
                last_name='Customer',
                phone_number=f'0912345{unique_id[:4]}',
            ...
            category = Category.objects.create(
                name=f'Test Category {unique_id[:4]}',
                name_persian=f'دسته تست {unique_id[:4]}'
            )
            
            jewelry_item = JewelryItem.objects.create(
                name=f'Test Ring {unique_id[:4]}',
                sku=f'TEST-RING-{unique_id[:8]}',
            ...
            offline_storage = POSOfflineService.store_offline_transaction(
                ...
                device_id=f'test-device-{unique_id[:4]}'
            )
